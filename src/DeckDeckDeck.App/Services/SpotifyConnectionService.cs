...
public sealed class SpotifyConnectionService : ISpotifyConnectionService
{
    ...
    internal SpotifyConnectionService(
        SettingsService settingsService,
        IUrlLaunchService urlLaunchService,
    ...
    public async Task<SpotifyConnectionResult> ConnectAsync(
        string clientId,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(clientId))
        {
            return new SpotifyConnectionResult(false, "Spotify Client ID를 입력해 주세요.");
        }
        ...
        try
        {
            ...
            {
                return new SpotifyConnectionResult(false, "Spotify 로그인 페이지를 열지 못했습니다.");
            }

            var callback = await callbackTask;
            if (!callback.Succeeded || string.IsNullOrWhiteSpace(callback.Code))
            {
                return new SpotifyConnectionResult(false, callback.ErrorMessage ?? "Spotify 인증에 실패했습니다.");
            }

            var token = await ExchangeCodeAsync(trimmedClientId, redirectUri, callback.Code, codeVerifier, cancellationToken);
            if (token is null)
            {
                return new SpotifyConnectionResult(false, "Spotify 토큰을 받지 못했습니다.");
            }
        ...
        {
            return new SpotifyConnectionResult(false, "Spotify 서버에 연결하지 못했습니다. 네트워크를 확인해 주세요.");
        }
        catch (JsonException)
        {
            return new SpotifyConnectionResult(false, "Spotify 응답을 읽지 못했습니다.");
        }
        catch (TaskCanceledException)
        {
            return new SpotifyConnectionResult(false, "Spotify 연결 시간이 초과되었습니다.");
        }
    }
