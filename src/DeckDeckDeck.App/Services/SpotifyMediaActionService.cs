...
public sealed class SpotifyMediaActionService : ISpotifyMediaActionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return new SpotifyMediaActionResult(false, "Spotify를 다시 연결해 주세요.");
        }

        var tokenResult = await GetValidAccessTokenAsync(settings, cancellationToken);
        if (!tokenResult.Succeeded)
        {
            return new SpotifyMediaActionResult(false, tokenResult.ErrorMessage);
        }
        ...
        if (result.StatusCode is HttpStatusCode.Unauthorized)
        {
            ...
            {
                return new SpotifyMediaActionResult(false, refreshed.ErrorMessage);
            }
        ...
        HttpStatusCode? StatusCode)
    {
        ...
        public SpotifyMediaActionResult ToMediaActionResult()
        {
            return Succeeded
                ? new SpotifyMediaActionResult(true)
                : new SpotifyMediaActionResult(false, ErrorMessage);
        }
    }
}
