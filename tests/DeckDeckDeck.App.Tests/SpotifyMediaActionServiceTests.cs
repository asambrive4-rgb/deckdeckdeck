...
public sealed class SpotifyMediaActionServiceTests
{
    [Fact]
    public async Task ExecuteReturnsReconnectMessageWhenSpotifyIsDisconnected()
    {
        ...
        var service = new SpotifyMediaActionService(services.SettingsService, new HttpClient(handler));
    ...
    public async Task ExpiredTokenRefreshesBeforeExecutingCommand()
    {
        ...
        Assert.Equal("new-access", services.SettingsService.Load().SpotifyAccessToken);
    }
    ...
    public async Task OpenSpotifyAndResumeStopsWhenSpotifyAppLaunchFails()
    {
        ...
        SaveSpotifySettings(services, expiresAt: DateTimeOffset.UtcNow.AddHours(1));
        var handler = new QueueHttpMessageHandler();
        var appLaunchService = new StubSpotifyAppLaunchService { Result = false };
        var service = CreateSpotifyMediaActionService(services, handler, appLaunchService);
    ...
    public async Task OpenSpotifyAndResumePollsDevicesAndTransfersInactiveComputerDevice()
    {
        ...
        handler.Enqueue(HttpStatusCode.NoContent);
        var appLaunchService = new StubSpotifyAppLaunchService();
        var service = CreateSpotifyMediaActionService(services, handler, appLaunchService, attempts: 2);
    ...
    public async Task OpenSpotifyAndResumePrefersComputerDeviceOverActivePhone()
    {
        ...
        handler.EnqueueJson(
            ...
            """);
        handler.Enqueue(HttpStatusCode.NoContent);
        var service = CreateSpotifyMediaActionService(services, handler, new StubSpotifyAppLaunchService());
    ...
    public async Task OpenSpotifyAndResumeReturnsFriendlyMessageWhenNoDeviceAppears()
    {
        ...
        handler.EnqueueJson(HttpStatusCode.OK, """{"devices":[]}""");
        handler.EnqueueJson(HttpStatusCode.OK, """{"devices":[]}""");
        var service = CreateSpotifyMediaActionService(
            services,
            handler,
            new StubSpotifyAppLaunchService(),
            attempts: 2);
    ...
    private static SpotifyMediaActionService CreateSpotifyMediaActionService(
        TestServices services,
        QueueHttpMessageHandler handler,
        ISpotifyAppLaunchService appLaunchService,
        int attempts = 10)
    {
        return new SpotifyMediaActionService(
    ...
    private sealed class StubSpotifyAppLaunchService : ISpotifyAppLaunchService
    {
