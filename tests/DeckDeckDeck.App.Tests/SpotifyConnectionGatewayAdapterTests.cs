...
public sealed class SpotifyConnectionGatewayAdapterTests
{
    [Fact]
    public async Task ConnectUsesPkceAndReturnsTokensWithoutSavingSettings()
    {
        ...
        var service = new SpotifyConnectionGatewayAdapter(
            urlLaunchService,
        ...
        Assert.Equal("Spotify 계정", result.DisplayName);
        Assert.Equal(string.Empty, settings.SpotifyClientId);
        Assert.Equal(string.Empty, settings.SpotifyAccessToken);
        Assert.Equal(string.Empty, settings.SpotifyRefreshToken);
    }
